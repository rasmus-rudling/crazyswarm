...
class CrazyflieController:
    ...
    def land(self, velocity=None):
        ...
        landedPose = Pose(currentPose.x,
                          currentPose.y,
                          landingHeight,
                          yaw=currentPose.yaw)

        landingTrejectory = self.getTrajectoryToPose(landedPose)
        landingTrejectory.slowTrajectory(factor=3)

        recordedPosesDuringLanding, recordedTrajectoryDuringLanding = self.followTrajectory(
            landingTrejectory)
    ...
    def hover(self, duration):
        ...
        recordedTrajectory = SimpleTrajectory(timestamps=timestamps,
                                              poses=recordedPosesDuringHover)
    ...
    def followTrajectory(self,
        ...
        trajectoryRange = range(numStates -
                                1, -1, -1) if reverse else range(numStates)

        for i in trajectoryRange:
            xTarget, yTarget, zTarget = trajectory.x[i], trajectory.y[
                i], trajectory.z[i]
        ...
        recordedTrajectory = SimpleTrajectory(timestamps=trajectory.timestamps,
                                              poses=recordedPoses)
    ...
    def getTrajectoryToPose(self, goalPose, velocity=None):
        ...
        x = np.linspace(currentPose.x, goalX, numValues)
        y = np.linspace(currentPose.y, goalY, numValues)
        z = np.linspace(currentPose.z, goalZ, numValues)
        yaw = np.linspace(currentPose.yaw, goalPose.yaw, numValues)
        timesteps = np.arange(0, duration, self.delta)

        minNumValues = np.min([
            x.shape[0], y.shape[0], z.shape[0], yaw.shape[0],
            timesteps.shape[0]
        ])
        x = x[:minNumValues]
        y = y[:minNumValues]
...
def flight1(droneController: "CrazyflieController"):
    ...
    trajetoryToStartPose = droneController.getTrajectoryToPose(startingPose,
                                                               velocity=0.5)
...
def flight2(droneController: "CrazyflieController"):
    landAndHovertrajectory = SimpleTrajectory(
        csv="hoverAndLandDataInMeters.csv")
    ...
    starting_offset = np.array([
        currentDronePose.x, currentDronePose.y, currentDronePose.z
    ]) - np.array([x_start_recorded, y_start_recorded, z_start_recorded])

    recordedPoses = droneController.followTrajectory(interpolatedTrajectory,
                                                     error=starting_offset)
    recordedTrajectory = SimpleTrajectory(
        timestamps=interpolatedTrajectory.timestamps, poses=recordedPoses)
...
def flight3(droneController: "CrazyflieController"):
    ...
    trajecoryToGoal = droneController.getTrajectoryToPose(goalPose,
                                                          velocity=1.25)
