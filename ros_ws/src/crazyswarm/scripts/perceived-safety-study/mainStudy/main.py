...
def getSf3(participantId):
    with open(f"{PATH_TO_ROOT}/mainStudy/participants/{participantId}/sf3.{participantId} - input.csv", newline='') as csvfile:
        lastRow = list(csv.reader(csvfile, delimiter=','))[-1]
...
def calcSF1():
    ...
    currentDroneState = DroneState(
        ...
        x=initDroneState.x, 
        y=initDroneState.y,
        yaw=planner.getYawForInitDroneState(initDroneState, goalState)
    )

    finalDroneState, foundGoalState = planner.findPathToGoal(currentDroneState, goalState)
    ...
    if foundGoalState:
        ...
        trajectory.plot(f"{filePath}/trajectoryPlot.png", planner.sf, planner=planner)
...
def calcSF3():
    ...
    if foundGoalState:
        ...
        recordFinalTrajectory(finalDroneState, planner, onlyFinalFrame=True, fileName="trajectory")
