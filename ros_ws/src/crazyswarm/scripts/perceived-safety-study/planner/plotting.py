...
def plotFlyZone(ax: Axes, flyZone: "FlyZone"):
  ...
  flyZonePatch = patches.Rectangle((flyZone.minPos.x, flyZone.minPos.y), zoneWidth, zoneHeight, edgecolor='r', facecolor='none', zorder=60)
  ...
  if flyZone.flyZoneWithPadding is not None:
    ...
    flyZoneWithPaddingPatch = patches.Rectangle(
      (flyZone.flyZoneWithPadding.minPos.x, flyZone.flyZoneWithPadding.minPos.y), 
      paddingZoneWidth, 
      paddingZoneHeight, 
      edgecolor='b', 
      facecolor='none',
...
def plotObstacles(ax: Axes, obstacles: List["Obstacle"], color:str="red", text:str="Obstacle", zorder:int=50):
  """Plot all obstacles.
  ...
  for obstacle in obstacles:
    ...
    patch = Circle(obstacleCenter, obstacle.radius, color=color, zorder=zorder)
    ax.add_artist(patch)

    ax.annotate(text, obstacleCenter, color='w', weight='bold', fontsize=9, ha='center', va='center', zorder=zorder+1)


def plotDroneState(ax: Axes, droneState: "DroneState", color:str="#232323", text:str="D") -> None:
  ...
  dronePatch = Circle(droneCenter, droneRadius, color=color, zorder=MAX_Z_ORDER)

  xMovement, yMovement = getDirectionVectorFromAngleAndLength(angle=droneState.yaw, length=droneRadius+0.11)
...
def savePlot(fig: Figure, folderNameForAnimation, animationName, saveInRoot=False) -> None:
  ratio = 30 / 24
  width_in_cm = 25
    ...
    try:
      ...
      try:
        ...
        try:
          ...
          try:
            fig.savefig(f"{PATH_TO_ROOT}/mainStudy/participants/{folderNameForAnimation}/{animationName}.png", format='png', dpi=120)
          except:
            fig.savefig(f"{PATH_TO_ROOT}/{folderNameForAnimation}/trajectory.png", format='png', dpi=120)

  else:
    fig.savefig(f"savedTrajectories/{folderNameForAnimation}/animationFrames/{animationName}.png", format='png', dpi=120)


def plotPathToCurrentDroneStateLive(droneState: "DroneState", planner: "Planner", ax: Axes, fig: Figure, isFinalDroneState: bool, fileName=None) -> bool:
  startX, startY = round(planner.currentStartState.x, 2), round(planner.currentStartState.y, 2)
  startPos = f"p(S)=({startX}, {startY})"

  goalX, goalY = round(planner.currentGoalState.x, 2), round(planner.currentGoalState.y, 2)
  ...
  plt.title(f"{computationTime}\n\n{startPos} ➙ {goalPos}\n\ni={planner.iterations} | D={droneState.depth} | {currentPos} | {currentVel} | {movementCost} | {distanceToHuman}", fontsize=12)
  plotFlyZone(ax, flyZone=planner.flyZone)
  plotObstacles(ax, obstacles=planner.obstacles)
  plotObstacles(ax, obstacles=[planner.HUMAN], color="blue", text="Human", zorder=100)
  plotPosition(ax, position=planner.currentGoalState, radius=planner.currentGoalState.radius)
  plotPathToCurrentState(ax, droneState, isFinalDroneState=isFinalDroneState, planner=planner)
  ...
  if planner.saveAnimation:
    ...
    savePlot(fig, folderNameForAnimation=planner.animationKey, animationName=animationName, saveInRoot=saveInRoot)
  
  plt.pause(0.0000001)
  plt.show()


def plotPathFromPreviousStateToCurrentState(ax: Axes, currentDroneState: "DroneState", previousDroneState: "DroneState", planner) -> bool:
  ...
  plotPosition(ax, previousDroneState, 0.1, color=color, text=f"{previousDroneState.depth}", zorder=previousDroneState.depth * 2 + 100)

  connectionPatch = ConnectionPatch(
    xyA=(previousDroneState.x, previousDroneState.y), coordsA="data",
    xyB=(currentDroneState.x, currentDroneState.y), coordsB="data",
    arrowstyle ="-",
    color=color,
    zorder=0
  )
...
def plotPathToCurrentState(ax: Axes, droneState: "DroneState", isFinalDroneState: bool, planner) -> bool:
  ...
  while currentDroneState.parent is not None:
    plotPathFromPreviousStateToCurrentState(ax, currentDroneState, currentDroneState.parent, planner)

    currentDroneState = currentDroneState.parent


def plotPathToCurrentDroneState(planner: "Planner", droneState: "DroneState") -> bool:
  ax = plt.subplots()[1]
  
  dt = 0.001

  while True:
    planner.plotStateLive(
...
def plotPosition(ax: Axes, position: "Position", radius: float, color:str="#16a34a", text:str="Goal", zorder:int=0):
  ...
  dronePatch = Circle(posCenter, radius, facecolor=color, edgecolor=edgecolor, zorder=zorder)
  ax.add_patch(dronePatch)
  ax.annotate(text, posCenter, color='w', weight='bold', fontsize=9, ha='center', va='center', zorder=zorder+1)
