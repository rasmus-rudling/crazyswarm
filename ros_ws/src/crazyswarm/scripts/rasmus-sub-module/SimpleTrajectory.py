...
class SimpleTrajectory:
  def __init__(self, timestamps=None, x=None, y=None, z=None, yaw=None, csv=None, poses=None, z_height=None):
    z_height = Z_HEIGHT if z_height is None else z_height
    ...
    if csv is not None:
      ...
      self.y = y
      print(z[0])
      self.z = z if z[0] != -1997 else np.full(len(x), z_height)  # If trajectory is from motion planner, keep z
      print(self.z)
      self.yaw = yaw
  ...
  def plotTrajectory(self, otherTrajectory: "SimpleTrajectory" = None, fileName = None):
    ...
    if otherTrajectory is not None and len(otherTrajectory.timestamps) > 0:
      ...
      axs[4].plot(otherTrajectory.timestamps, executedVelocities, "-r")
