...
def main():
  ...
  currentDroneState = Position(x=x2, y=y2)

  goalStates = [
    # DroneGoalState(x=x2, y=y2, radius=0.1),
    DroneGoalState(x=x1, y=y1, radius=0.1),
  ]

  for currentGoalState in goalStates:
    ...
    pr = cProfile.Profile()
    pr.enable()
    currentDroneState, foundGoalState = planner.findPathToGoal(currentDroneState, currentGoalState)
    pr.disable()
    s = io.StringIO()
    ...
    if foundGoalState:
      ...
      trajectory.plot(f"{filePath}/trajectoryPlot.png", cbf)
